...
The Killing--Cartan classification exhausts the finite-dimensional compact
simple Lie algebras: the four classical series \(A_n,B_n,C_n,D_n\) and the
five exceptional types \(G_2,F_4,E_6,E_7,E_8\) \cite[Ch.~VI]{Bourbaki}.
A \emph{simple} Lie algebra is nonabelian and has no nonzero proper ideal;
a \emph{semisimple} Lie algebra is a finite direct sum of simple Lie algebras.
Every finite-dimensional compact real Lie algebra is a direct sum of a
semisimple algebra and an abelian centre. Thus the starting domain includes
all compact algebra types, with no rank cutoff; the subsequent argument
determines which embeddings satisfy the two conditions.
