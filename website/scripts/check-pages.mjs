...
for (const id of [
  ...
  assert.equal(
    ...
    1,
    'Missing or duplicate learning section: ' + id,
  );
}
assert.match(doc.body.textContent, /How does the mathematics meet reality/);
assert.match(
  doc.body.textContent,
  /representation can be read using only the transformations/,
);
assert.doesNotMatch(
  doc.body.textContent,
  /paper states the interpretation explicitly|Identifying V with the full chiral matter module/,
);
let references = 0;
...
assert(
  !existsSync(resolve(output, 'chirality-standard-model.zip')),
  'Do not ship a second stale repository archive',
);
console.log(
  `PASS: static Pages export; ${references} local references; ${manifest.files.length} byte-identical downloads; ${files.length} public files; metadata and current argument text.`,
);
